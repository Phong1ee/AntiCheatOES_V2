...
class ExamSetting(Base):
    ...
    sequential_navigation: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False, server_default=text("0")
    )
...
class AttemptQuestion(Base):
    ...
    question_type_snapshot: Mapped[Optional[str]] = mapped_column(
        String(30),
        nullable=True,
    )
    ...
    options_snapshot: Mapped[Optional[list]] = mapped_column(
        JSON,
        nullable=True,
    )
...
class MCQAnswer(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "attempt_id",
            "question_id",
            name="uq_mcq_answers_attempt_question",
        ),
    )
