...
from src.service.anti_cheat_event_policy import classify_event
...
def decorate_timeline(events):
    for event in events:
        event.update(classify_event(event["eventType"]))
    return events

def decorate_attempt_summaries(attempt_rows):
    for attempt in attempt_rows:
        events = rows("SELECT event_type eventType FROM exam_event WHERE attempt_id=%s", (attempt["attemptId"],))
        automated_count = sum(classify_event(event["eventType"])["automatedFlag"] for event in events)
        attempt["flagged"] = automated_count > 0
        attempt["aiFlagCount"] = automated_count
    return attempt_rows
...
def attempts(exam_id:int, search:str="", status:str="", limit:int=Query(50,ge=1,le=100), offset:int=Query(0,ge=0), user=Depends(teacher)):
    ...
    attempt_rows = rows("""SELECT a.attempt_id attemptId,a.student_id studentId,u.full_name studentName,a.attempt_no attemptNo,a.status attemptStatus,a.score,a.violation_count violationCount,COALESCE(es.violation_limit,5) violationLimit,a.termination_reason terminationReason,
    ...
    return decorate_attempt_summaries(attempt_rows)
...
def detail(attempt_id:int,user=Depends(teacher)):
    ...
    timeline=decorate_timeline(rows("SELECT event_type eventType,event_timestamp eventTimestamp,source,details,metadata,is_violation isViolation FROM exam_event WHERE attempt_id=%s ORDER BY event_timestamp,event_id",(attempt_id,)))
    return {"attempt":base[0],"breakdown":rows("SELECT event_type eventType,COUNT(*) count FROM exam_event WHERE attempt_id=%s AND is_violation=1 GROUP BY event_type",(attempt_id,)),"timeline":timeline}
