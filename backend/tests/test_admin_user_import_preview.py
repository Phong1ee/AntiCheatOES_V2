...
HEADERS = ["full_name", "email", "role", "phone", "date_of_birth", "initial_password"]
...
class AdminUserImportPreviewTests(unittest.TestCase):
    ...
    def test_valid_dates_and_zero_writes(self):
        rows = parse_user_import_xlsx(workbook_bytes([
            ["Student", "STUDENT@example.test", "student", None, date(2005, 4, 18), "password123"],
            ["Admin", "admin2@example.test", "ADMIN", "0123456789", "2000-01-02", "password123"],
        ]))
    ...
    def test_duplicates_and_existing_soft_deleted_users_block_rows(self):
        ...
        self.db.commit()
        # Include school_id column to test duplicate/existing detection (optional in normal flow)
        rows = parse_user_import_xlsx(workbook_bytes([
            ["S1", "One", "one@example.test", "student", None, None, "password123"],
            ["S1", "Two", "ONE@example.test", "student", "bad", "not-a-date", "short"],
    ...
    def test_rejects_missing_sheet_header_and_malformed_workbook(self):
        ...
        with self.assertRaises(UserImportParseError): parse_user_import_xlsx(workbook_bytes([], headers=["full_name"]))
    ...
    def test_import_creates_mixed_users_with_hashed_passwords(self):
        result = import_users_from_rows(parse_user_import_xlsx(workbook_bytes([
            ["Student", "student@example.test", "student", None, "2005-04-18", "password123"],
            ["Teacher", "teacher@example.test", "teacher", "0123456789", None, "password456"],
    ...
    def test_import_rejects_all_rows_when_one_is_invalid(self):
        rows = parse_user_import_xlsx(workbook_bytes([
            ["One", "one@example.test", "student", None, None, "password123"],
            ["Two", "one@example.test", "student", None, None, "password123"],
        ]))
    ...
    def test_database_integrity_error_rolls_back_the_entire_batch(self):
        rows = parse_user_import_xlsx(workbook_bytes([
            [f"User {index}", f"user{index}@example.test", "student", None, None, "password123"]
