...
from src.route.teacherRoute.antiCheatRoute import decorate_timeline
...
def detector_metadata():
    return {
        "detectorName": "test-detector", "detectorVersion": "1", "modelVersion": "1",
        "policyVersion": "1", "incidentId": "incident-1", "confidence": 0.9,
    }
...
class AntiCheatEventTests(unittest.TestCase):
    ...
    def test_review_only_event_is_persisted_without_incrementing_counter(self):
        cursor = _EventCursor(limit=1)
        result = self._record(cursor, event("SPEECH_ACTIVITY_DETECTED", "speech-1", "microphone", detector_metadata()))
        self.assertTrue(result["eventAccepted"])
        self.assertEqual(result["violationCount"], 0)
        self.assertFalse(result["terminated"])
        self.assertTrue(result["automatedFlag"])
        self.assertFalse(result["countsTowardLimit"])

    def test_policy_rejects_source_mismatch_and_invalid_metadata(self):
        with self.assertRaises(ValidationError):
            AntiCheatEventRequest(
                attemptId=10, clientEventId="wrong-source", eventType="PHONE_DETECTED", source="microphone",
                metadata=detector_metadata(),
            )
        with self.assertRaises(ValidationError):
            AntiCheatEventRequest(
                attemptId=10, clientEventId="raw-media", eventType="PHONE_DETECTED", source="camera",
                metadata={**detector_metadata(), "videoFrame": "data:image/png;base64,abc"},
            )
        with self.assertRaises(ValidationError):
            AntiCheatEventRequest(
                attemptId=10, clientEventId="bad-score", eventType="PHONE_DETECTED", source="camera",
                metadata={**detector_metadata(), "confidence": 1.1},
            )

    def test_limit_terminates_once_with_zero_score(self):
        cursor = _EventCursor(limit=1)
        terminated = self._record(cursor, event("PHONE_DETECTED", "event-1", "camera", detector_metadata()))
    ...
    def test_teacher_timeline_uses_the_shared_policy_classification(self):
        timeline = decorate_timeline([
            {"eventType": "PHONE_DETECTED"},
            {"eventType": "SPEECH_ACTIVITY_DETECTED"},
            {"eventType": "ATTEMPT_TERMINATED"},
        ])
        self.assertEqual(timeline[0], {"eventType": "PHONE_DETECTED", "automatedFlag": True, "countsTowardLimit": True})
        self.assertEqual(timeline[1], {"eventType": "SPEECH_ACTIVITY_DETECTED", "automatedFlag": True, "countsTowardLimit": False})
        self.assertEqual(timeline[2], {"eventType": "ATTEMPT_TERMINATED", "automatedFlag": False, "countsTowardLimit": False})
