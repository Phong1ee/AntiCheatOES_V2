...
export class PerformanceController {
  ...
  get targetFps(): number {
    if (this.kind === 'face') {
      if (this.averageMs < 120) return Math.min(FACE_FPS[0], CAMERA_AI_CONFIG.mainThreadMaxFps);
      if (this.averageMs < 220) return Math.min(FACE_FPS[1], CAMERA_AI_CONFIG.mainThreadMaxFps);
