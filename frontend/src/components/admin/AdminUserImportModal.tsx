...
export function AdminUserImportModal({ open, onClose, onImported }: AdminUserImportModalProps) {
  ...
  useEffect(() => {
    if (!open) { setFile(null); setPreview(null); setResult(null); setError(null); setIsPreviewing(false); setIsImporting(false); }
  }, [open]);
  ...
  const importFile = async () => {
    ...
    try { const importResult = await adminUserService.importUsers(file); setResult(importResult); await onImported(); toast.success(`${importResult.imported_count} users imported successfully.`); }
    catch (requestError) { setError(requestError instanceof Error ? requestError.message : "Unable to import this file."); }
  ...
  return <Dialog open={open} onOpenChange={(nextOpen) => { if (!nextOpen && !isBusy) onClose(); }}>
    <DialogContent className="max-w-5xl gap-0 overflow-hidden p-0" onEscapeKeyDown={(event) => { if (isBusy) event.preventDefault(); }} onPointerDownOutside={(event) => { if (isBusy) event.preventDefault(); }}>
      <DialogHeader className="border-b border-gray-100 px-6 py-5 text-left">
        ...
        <div className="mt-3 flex gap-4 text-xs font-medium text-gray-500">{["Upload", "Preview", "Import Result"].map((label, index) => <span key={label} className={index <= step ? "text-teal-700" : ""}>{index + 1}. {label}</span>)}</div>
      </DialogHeader>
      ...
      <div className="flex justify-end gap-2 border-t border-gray-100 px-6 py-4"><button disabled={isBusy} onClick={onClose} className="rounded-lg border px-4 py-2 text-sm">{result ? "Close" : "Cancel"}</button>{!preview && !result && <button disabled={!file || isBusy} onClick={() => void previewFile()} className="rounded-lg bg-teal-600 px-4 py-2 text-sm text-white disabled:opacity-50">{isPreviewing && <LoaderCircle className="mr-1 inline size-4 animate-spin" />}Preview</button>}{preview && !result && <button disabled={preview.error_count > 0 || isBusy} onClick={() => void importFile()} className="rounded-lg bg-teal-600 px-4 py-2 text-sm text-white disabled:opacity-50">{isImporting && <LoaderCircle className="mr-1 inline size-4 animate-spin" />}Import Users</button>}</div>
    </DialogContent>
  </Dialog>;
}
