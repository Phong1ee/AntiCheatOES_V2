...
const CAMERA_PREVIEW_WIDTH = 216;
...
export function WebcamMonitor({ stream }: { stream: MediaStream }) {
  ...
  const cameraPreviewHeight = Math.round(CAMERA_PREVIEW_WIDTH / cameraAspectRatio);
  ...
  const drag = (event: PointerEvent<HTMLDivElement>) => {
    ...
    const height = cameraPreviewHeight;
    setPosition({
  ...
  return (
    <div
      ...
      style={{ left: position.x, top: position.y, width: CAMERA_PREVIEW_WIDTH, height: cameraPreviewHeight }}
    ...
    >
      <div className="relative h-full overflow-hidden rounded-xl bg-slate-950 shadow-lg">
        {cameraLive ? <video ref={videoRef} autoPlay playsInline muted className="h-full w-full object-contain" /> : <div className="flex h-full items-center justify-center text-slate-300"><CameraOff className="size-6" /></div>}
