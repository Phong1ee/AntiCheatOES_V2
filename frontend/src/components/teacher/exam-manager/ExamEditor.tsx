...
export function ExamEditor({ examId, exam, subjects, initialTab, onClose, onSave, onResultVisibilityChange, onStatusChange, onViewInQuestionBank, onDirtyChange }: ExamEditorProps) {
        ...
        <div className={`flex-1 min-h-0 ${activeTab === 'questions' ? 'overflow-hidden' : 'overflow-y-auto'}`}>
          ...
          {visitedTabs.includes('assignment') && (
            <TabsContent value="assignment" forceMount className="m-0 p-6 data-[state=inactive]:hidden">
              <AssignmentTab examId={examId} onDirtyChange={handleAssignmentDirty} />
            </TabsContent>
          )}
