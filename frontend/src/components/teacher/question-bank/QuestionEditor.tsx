...
export function QuestionEditor({
  ...
  const toggleLo = (loId: number, checked: boolean) => {
    setLoIds((current) =>
      checked ? [...current, loId] : current.filter((id) => id !== loId),
    );
  };
          ...
          {!loading && (
            <>
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="min-h-24 space-y-2 rounded-lg border border-gray-200 bg-gray-50 p-3">
                      {chapterIds.length === 0 && (
                        <p className="text-sm text-gray-500">
                          Select at least one chapter to load learning
                          objectives.
                        </p>
                      )}
                      ...
                      {chapterIds.length > 0 &&
                        learningObjectives.length === 0 && (
                          <p className="text-sm text-gray-500">
                            No learning objectives are available for selected
                            chapters.
                          </p>
                        )}
              ...
              <section className="space-y-3 rounded-xl border border-gray-200 p-5">
                ...
                {questionType === "essay" && (
                  <p className="text-sm text-gray-500">
                    No suggested answer field exists in the current schema.
                  </p>
                )}
              </section>
