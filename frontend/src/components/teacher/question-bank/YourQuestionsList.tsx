...
function getRejectedFeedback(question: QuestionBankItem): string | null {
  return (question as QuestionBankItem & { rejected_feedback?: string | null }).rejected_feedback ?? null;
}
...
export function YourQuestionsList({
  ...
  return (
    <section className="yq-ref-root" aria-label="Your questions">
      <div className="yq-ref-status-tabs" role="tablist" aria-label="Question status">
        {statusTabs.map((tab) => {
          ...
          return (
            ...
            >
              <span>{tab.label}</span>
              {typeof count === 'number' && (
                <span className={`yq-ref-status-count status-${tab.value}`}>{count}</span>
              )}
            </button>
          );
        })}
      ...
      <div className="yq-ref-toolbar">
        <p className="yq-ref-total">
          <strong>{loading ? 0 : total}</strong> question{total === 1 ? '' : 's'}
        </p>

        <button type="button" className="yq-ref-new-button" onClick={onNewQuestion}>
        ...
        <>
          <div className="yq-ref-list">
            {questions.map((question) => {
              ...
              const chapterText = joinValues(question.chapters.map((chapter) => chapter.chapter_name));
              const loText = joinValues(question.learning_objectives.map((lo) => lo.lo_name));
              const showOptionCount =
                question.question_type === 'MCQ' && typeof question.option_count === 'number';
                ...
                >
                  <div className="yq-ref-card-main">
                    ...
                    <div className="yq-ref-card-content">
                      <p className="yq-ref-question-text">{question.question_text}</p>

                      <div className="yq-ref-pills">
                        <span className={`yq-ref-pill type-${type.className}`}>
                          {type.label}
                          {showOptionCount && (
                            <span className="yq-ref-pill-muted">· {question.option_count} opts</span>
                          )}
                        </span>

                        {difficulty && (
                          <span className={`yq-ref-pill difficulty-${difficulty.className}`}>
                            <span className="yq-ref-difficulty-dot" />
                            {difficulty.label}
                          </span>
                        )}

                        <span className={`yq-ref-pill status-${status.className}`}>
                      ...
                      <div className="yq-ref-expanded-inner">
                        <div className="yq-ref-metadata-grid">
                          ...
                          <div className="yq-ref-meta-item">
                            ...
                            <div>
                              ...
                              {question.learning_objectives.length > 0 && (
                                <p className="yq-ref-meta-secondary">LO: {loText}</p>
                              )}
                            </div>
                          </div>
                          ...
                          <div className="yq-ref-meta-item">
                            ...
                            <div>
                              <p className="yq-ref-meta-label">{updatedDate ? 'Updated' : 'Created'}</p>
                              <p className="yq-ref-meta-value">{displayDate ?? 'Not available'}</p>
                            </div>
                          </div>
                        </div>
                          ...
                          {question.permissions.can_delete && (
                            <AlertDialog>
                              <AlertDialogTrigger asChild>
                                <button type="button" className="yq-ref-action-button is-delete">
